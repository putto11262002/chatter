...
func (s *SQLiteChatStore) CreateRoom(ctx context.Context, name string, ownerUsername string) (string, error) {
	...
	_, err = tx.ExecContext(ctx, query,
		...
		sql.Named("role", models.Owner), sql.Named("last_message_read", 0))
	if err != nil {
...
func (s *SQLiteChatStore) AddRoomMember(ctx context.Context, roomID, username string, role models.MemberRole) error {
...
func (s *SQLiteChatStore) GetRoomByID(ctx context.Context, roomID string) (*models.Room, error) {
	...
	members := make([]models.RoomMember, 0, 2)
...
func (s *SQLiteChatStore) IsRoomMember(ctx context.Context, roomID, user string) (bool, models.MemberRole, error) {
	query := `
	...
	role := models.MemberRole(_role.String)

	return true, role, nil
}

func (s *SQLiteChatStore) GetUserRooms(ctx context.Context, user string, offset, litmit int) ([]models.Room, error) {
	...
	WITH r as (
	    ...
	    ORDER BY r.last_message_sent_at DESC
	...
	rooms := make([]models.Room, 0, len(roomMap))
