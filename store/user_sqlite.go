...
func (s *SQLiteUserStore) GetUsersByUsernames(ctx context.Context, usernames ...string) ([]models.UserWithoutSecrets, error) {
	if len(usernames) == 0 {
		return nil, nil
	}
	...
	var users []models.UserWithoutSecrets

	for rows.Next() {
		user := models.UserWithoutSecrets{}
...
func (s *SQLiteUserStore) GetUserByUsername(ctx context.Context, username string) (*models.UserWithoutSecrets, error) {

	row := s.db.QueryRowContext(ctx, "SELECT name, username FROM users WHERE username = ? LIMIT 1", username)

	user := new(models.UserWithoutSecrets)
...
func (s *SQLiteUserStore) GetUsers(ctx context.Context, options *GetUsersOptions) ([]models.UserWithoutSecrets, error) {
	...
	users := []models.UserWithoutSecrets{}

	for row.Next() {
		var user models.UserWithoutSecrets
